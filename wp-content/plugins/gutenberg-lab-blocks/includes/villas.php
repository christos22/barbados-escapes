...
function gutenberg_lab_blocks_get_villa_amenity_icon_registry() {
	...
	return array(
		...
		'bathtub-thick'      => array(
			'label' => __( 'Bathtub Thick', 'gutenberg-lab-blocks' ),
			'asset' => $theme_asset_dir . 'bathtub-thick.svg',
		),
		'bbq'                => array(
		...
		'bedrooms-line'      => array(
			'label'    => __( 'Bedrooms Line', 'gutenberg-lab-blocks' ),
			'paths'    => '<path d="M4 18.5V9.2c0-1.4 1.1-2.5 2.5-2.5h11c1.4 0 2.5 1.1 2.5 2.5v9.3"></path><path d="M4 14.5h16"></path><path d="M6.2 14.5v-3.1c0-.8.6-1.4 1.4-1.4h3c.8 0 1.4.6 1.4 1.4v3.1"></path><path d="M12 14.5v-3.1c0-.8.6-1.4 1.4-1.4h3c.8 0 1.4.6 1.4 1.4v3.1"></path><path d="M4 18.5h16"></path><path d="M6 18.5V20M18 18.5V20"></path>',
			'view_box' => '2.8 5.5 18.4 15.5',
		),
		'breakfast-bar'      => array(
 ...
 * @return string
 */
function gutenberg_lab_blocks_format_villa_search_term_label( $term_name ) {
	$term_name = trim( (string) $term_name );
	$label     = preg_replace( '/\bVillas\b/ui', '', $term_name );

	if ( null === $label ) {
		return $term_name;
	}

	$label = preg_replace( '/^\s*with\s+/ui', '', $label );
...
/**
 * Maps the standard villa facts line to compact icon-and-label items.
 *
 * Card facts remain one editor-managed string, so the renderer only enhances
 * the three known fact types. If the string contains anything unexpected, an
 * empty result tells the card to preserve the original text instead of hiding
 * content it does not understand.
 *
 * @param string $facts    Normalized villa card facts.
 * @param int    $bedrooms Structured bedroom count used when legacy facts omit it.
 * @param int    $sleeps   Structured occupancy used when legacy facts omit it.
 * @return array<int, array{icon:string,label:string}>
 */
function gutenberg_lab_blocks_get_villa_card_fact_icon_items( $facts, $bedrooms = 0, $sleeps = 0 ) {
	$facts    = sanitize_text_field( (string) $facts );
	$bedrooms = absint( $bedrooms );
	$sleeps   = absint( $sleeps );

	if ( '' === $facts && 0 === $bedrooms && 0 === $sleeps ) {
		return array();
	}

	$matched = preg_match_all(
		'/\b(?:\d+(?:\.\d+)?\s+Bedrooms?|\d+(?:\.\d+)?\s+Bathrooms?|(?:Sleeps?|Guests?|Occupancy)\s*(?:up\s+to\s+)?\d+)\b/ui',
		$facts,
		$fact_matches
	);
	...
	$remaining_facts = preg_replace(
		'/\b(?:\d+(?:\.\d+)?\s+Bedrooms?|\d+(?:\.\d+)?\s+Bathrooms?|(?:Sleeps?|Guests?|Occupancy)\s*(?:up\s+to\s+)?\d+)\b/ui',
		'',
		$facts
	);

	if ( ! is_string( $remaining_facts ) || preg_match( '/[^\s,|\/–—-]/u', $remaining_facts ) ) {
		return array();
	}
	...
	if ( ! isset( $items['people'] ) && $sleeps > 0 ) {
		$items['people'] = sprintf(
			/* translators: %s: maximum villa occupancy. */
			__( 'Sleeps %s', 'gutenberg-lab-blocks' ),
			number_format_i18n( $sleeps )
		);
	}
...
function gutenberg_lab_blocks_get_villa_data( $villa_id ) {
	...
	$bedrooms   = absint( get_post_meta( $villa_id, 'villa_search_bedrooms', true ) );
	$sleeps     = absint( get_post_meta( $villa_id, 'villa_search_sleeps', true ) );
...
function gutenberg_lab_blocks_render_villa_card( $villa_id, $args = array() ) {
	...
	$new_comparison_fact_styles = array(
		'inline_gold_icons_lg',
	...
	$is_inline_fact_style = in_array(
		$fact_style,
		array_merge( array( 'inline', 'inline_faint', 'inline_faint_facts' ), $new_comparison_fact_styles ),
		true
	);
	...
	if ( 'collection' === $args['presentation'] ) {
		$explore_button_class = $uses_outline_explore_cta
			? 'is-style-outline'
			: 'is-style-vvm-link-primary';
	}

	if ( $uses_glow_only_explore_cta ) {
		$explore_button_class .= ' vvm-card-grid__explore-cta--glow-only';
	}
	...
	if ( $fact_icon_items ) {
		...
		if ( $is_inline_fact_style ) {
			$card_classes[] = 'vvm-card-grid__card--facts-inline-layout';
		}

		if ( in_array( $fact_style, $new_comparison_fact_styles, true ) ) {
			$card_classes[] = 'vvm-card-grid__card--fact-strip-roomy';
			$card_classes[] = $uses_xlarge_fact_icons
				? 'vvm-card-grid__card--fact-icons-xlarge'
				: 'vvm-card-grid__card--fact-icons-large';
		}

		if ( $uses_gold_fact_icons ) {
			$card_classes[] = 'vvm-card-grid__card--fact-icons-gold';
		}

		if ( $uses_green_fact_icons ) {
			$card_classes[] = 'vvm-card-grid__card--fact-icons-green';
		}

		if ( $uses_gold_fact_labels ) {
			$card_classes[] = 'vvm-card-grid__card--fact-labels-gold';
		}
		...
		if ( $is_version_thirteen ) {
			...
			$card_classes[] = 'vvm-card-grid__card--price-refined-sans';
		}
		...
		<div class="vvm-card-grid__card-content">
			...
			<?php if ( ! empty( $villa_data['cta']['url'] ) && ! empty( $villa_data['cta']['label'] ) ) : ?>
				<div class="wp-block-buttons<?php echo '' !== $args['enquiry_url'] ? ' vvm-card-grid__card-actions' : ''; ?>">
					<div class="wp-block-button <?php echo esc_attr( $explore_button_class ); ?>">
						<a
							class="wp-block-button__link wp-element-button<?php echo $uses_outline_explore_cta && ! $uses_glow_only_explore_cta ? ' has-dark-green-color has-text-color has-background has-border-color' : ''; ?>"
...
function gutenberg_lab_blocks_render_villa_hero_search_markup( $attributes = array(), $wrapper_attributes = '' ) {
	...
	$location_placeholder = isset( $attributes['locationPlaceholder'] ) && is_string( $attributes['locationPlaceholder'] )
		...
		: __( 'Area', 'gutenberg-lab-blocks' );
	// Preserve existing blocks while upgrading the former default prompt.
	if ( 'Search by Area' === $location_placeholder ) {
		$location_placeholder = __( 'Area', 'gutenberg-lab-blocks' );
	}
				...
				<div class="vvm-villa-hero-search__field">
					...
					>
						...
						<?php foreach ( $locations as $location ) : ?>
							...
							>
								<?php echo esc_html( gutenberg_lab_blocks_format_villa_search_term_label( $location->name ) ); ?>
							</option>
						<?php endforeach; ?>
					</select>
				</div>
				...
				<details class="vvm-villa-hero-search__field vvm-villa-hero-search__field--price">
					...
					<div class="vvm-villa-hero-search__price-panel">
						<label for="<?php echo esc_attr( $min_price_field_id ); ?>">
							<?php esc_html_e( 'Minimum USD / night', 'gutenberg-lab-blocks' ); ?>
						</label>
						<input
						...
						<label for="<?php echo esc_attr( $max_price_field_id ); ?>">
							<?php esc_html_e( 'Maximum USD / night', 'gutenberg-lab-blocks' ); ?>
						</label>
						<input
