...
if ( ! function_exists( 'gutenberg_lab_peeking_carousel_render_slide' ) ) {
	/**
	 * Renders one parsed slide block into the shared carousel slide shell.
	 *
	 * @param array  $slide_block Parsed child slide block.
	 ...
	 * @return string
	 */
	function gutenberg_lab_peeking_carousel_render_slide( $slide_block, $index, $slide_data_attribute ) {
		if ( ! is_array( $slide_block ) ) {
			return '';
		}
		...
		ob_start();
		?>
		<li class="splide__slide vvm-feature-carousel__slide-shell<?php echo 0 === $index ? ' is-active' : ''; ?>" <?php echo esc_attr( $slide_data_attribute ); ?>>
